...
public class DistributedQueryThenSteps {
    ...
    public void 兩個實例都應成功回傳預訂資料() {
        BookingResponse response1 = (BookingResponse) scenarioContext.get("response1");
        BookingResponse response2 = (BookingResponse) scenarioContext.get("response2");
    ...
    public void 兩個實例回傳的預訂資料應完全一致() {
        ...
        assertEquals(response1.getBookingId(), response2.getBookingId(), "bookingId 應相同");
    ...
    public void instance2應透過HTTP遠端查詢instance1() {
        ...
        verify(uriSpec).uri("http://localhost:" + port1 + "/internal/bookings/" + bookingId);
    }
}
