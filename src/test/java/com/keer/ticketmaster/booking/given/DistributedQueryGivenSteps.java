package com.keer.ticketmaster.booking.given;
...
public class DistributedQueryGivenSteps {
    ...
    @假如("^一筆預訂「(.+)」已寫入 booking-completed topic，使用者為「(.+)」，狀態為「(.+)」，座位為「(.+)」$")
    public void 一筆預訂已寫入topic(String bookingId, String userId, String status, String seatsStr) {
        ...
        when(kafkaStreams1.queryMetadataForKey(anyString(), eq(bookingId), any(Serializer.class)))
                .thenReturn(metadata);
        when(kafkaStreams2.queryMetadataForKey(anyString(), eq(bookingId), any(Serializer.class)))
                .thenReturn(metadata);

        // Instance 1 的本地 store 中有這筆預訂
        BookingCompletedEvent event = BookingCompletedEvent.newBuilder()
        ...
        ReadOnlyKeyValueStore<String, BookingCompletedEvent> store = mock(ReadOnlyKeyValueStore.class);
        when(kafkaStreams1.store(any())).thenReturn(store);
        when(store.get(bookingId)).thenReturn(event);

        // Instance 2 透過 RestClient 遠端查詢 instance1
        BookingResponse remoteResponse = BookingResponse.builder()
                .bookingId(bookingId)
                .eventId(1L)
                .userId(userId)
                .status(status)
        ...
        when(responseSpec.body(BookingResponse.class)).thenReturn(remoteResponse);
